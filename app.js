...
function setStatus(text, online = true) {
  ...
  statusPill.classList.remove('offline', 'connecting');
  if (!online) statusPill.classList.add('offline');
  else if (text.toLowerCase().includes('connect')) statusPill.classList.add('connecting');
  const fs = document.getElementById('footer-status');
  if (fs) fs.textContent = online ? 'Live' : 'Offline';
}
...
const darkAddBtn = document.getElementById('darkAddBtn');
if (darkAddBtn) darkAddBtn.addEventListener('click', openModal);
